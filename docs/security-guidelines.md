...
### Role Hierarchy
This repo scaffolds a four-tier RBAC structure under `config/rbac/`. All operator-created RBAC must follow this pattern:
...
- Default: no security context set (`legacy` mode) to maintain OpenShift compatibility across versions. OpenShift PSA applies `MustRunAsRange` automatically via namespace annotations.
